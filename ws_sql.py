...
db=MySQLdb.connect(user="root",passwd="weatherstation",db="weather")
c = db.cursor()
# Rainfall
rainfall_avs={}
for mon in range(1,13):
    c.execute("""select AVG(RAINFALL) from WEATHER_MEASUREMENT WHERE MONTH(CREATED) = %s AND YEAR(CREATED) = 2017;""",(mon,))
    r=c.fetchone()[0]
    if r:
...
rainfall_tots={}
for mon in range(1,13):
    c.execute("""select SUM(RAINFALL) from WEATHER_MEASUREMENT WHERE MONTH(CREATED) = %s AND YEAR(CREATED) = 2017;""",(mon,))
    r=c.fetchone()[0]
    if r:
